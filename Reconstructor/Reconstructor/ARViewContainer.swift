    ...
    private func setupViews() {
        ...
        if useRoomPlan {
            ...
            guard ARWorldTrackingConfiguration.isSupported else {
                DispatchQueue.main.async {
                    self.delegate?.didFailWithError(NSError(domain: "Reconstructor", code: 0, userInfo: [NSLocalizedDescriptionKey: "AR World Tracking is not supported on this device."]))
                }
                arView = av
                return
            }
            ...
            if ARWorldTrackingConfiguration.supportsFrameSemantics(.smoothedSceneDepth) {
                ...
                DispatchQueue.main.async {
                    self.delegate?.didFailWithError(NSError(domain: "Reconstructor", code: 1, userInfo: [NSLocalizedDescriptionKey: "LiDAR sensor not detected. Point cloud scanning requires a LiDAR-equipped device (iPhone/iPad Pro)."]))
                }
            }
