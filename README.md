...
| `/mapping/map` | `visualization_msgs/msg/MarkerArray` | ~0.5-2 Hz | Reconstructed triangular mesh markers (`TRIANGLE_LIST`) for instant Rviz2 display. The ARKit mesh publishes gray markers in namespace `mesh`; the calibrated Depth Anything overlay publishes a camera-colored marker (per-vertex `colors`) in namespace `depth_mesh`, id 0, deleted on scan stop. |
| `/mapping/mesh_snapshot` | `mapeverything_msgs/msg/MeshSnapshot` | ~0.5-2 Hz | Structured triangle-list mesh snapshot for rosbag recording, with base64 packed little-endian vertex/index bytes plus truncation and payload-size metadata. Schema v3 adds an optional colored vertex encoding for the Depth Anything overlay grid: `vertex_encoding = float32_xyz_rgb8_le_base64` / `vertex_stride_bytes = 15` marks 12-byte xyz followed by 3-byte rgb; the ARKit mesh keeps `float32_xyz_le_base64` / 12. Consumers switch on `vertex_encoding`. |
